...
public:
    ...
    void shutdown();
    void read(void* context) noexcept;
    ...
    inline unsigned long buffer_size() const noexcept { return _buffer_size; }
    
private:
    static void main_worker(server* serv) noexcept;

    HANDLE listen_to_pipe(bool* pending) noexcept;
    void accept(HANDLE pipe, OVERLAPPED* overlap, bool pending) noexcept;
...
public:
    ...
    virtual bool connect(const std::wstring& name, unsigned long timeout=3000) noexcept;
