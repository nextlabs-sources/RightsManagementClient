...
public:
    ...
    virtual const unsigned char* get_data(size_t& length) const = 0;
...
class raw_request : public basic_request
{
public:
    raw_request(const std::wstring& request_method,
        const std::wstring& request_path,
        const std::vector<std::pair<std::wstring, std::wstring>>& request_headers,
    ...
    virtual const unsigned char* get_data(size_t& length) const;
