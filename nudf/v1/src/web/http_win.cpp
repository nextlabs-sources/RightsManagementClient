...
#include <nudf\web\http_client_impl.hpp>
...
namespace details
{
    ...
    static std::wstring parse_reason_phrase(HINTERNET request_handle)
    {
        std::wstring phrase;
        DWORD length = 0;
    ...
    static std::string build_error_msg(unsigned long code, const std::string &location)
    {
        ...
        msg.append(": ");
        msg.append(utility::windows_category().message(code));
        return msg;
    }
    ...
    public:
        ...
        unsigned __int64 m_remaining_to_write;
    ...
    {
    public:
        winhttp_client(uri address, http_client_config client_config)
            : _http_client_communicator(std::move(address), std::move(client_config)), m_secure(m_uri.scheme() == L"https"), m_hSession(nullptr), m_hConnection(nullptr) { }
    ...
    protected:
        ...
        unsigned long report_failure(const std::wstring& errorMessage)
        {
            // Should we log?
        ...
        unsigned long open()
        {
            ...
            if (!m_hSession)
            {
                return report_failure(L"Error opening session");
            }

            if (config.secure_protocols() != 0) {
                unsigned long protos = config.secure_protocols();
                if (!::WinHttpSetOption(m_hSession, WINHTTP_OPTION_SECURE_PROTOCOLS, &protos, sizeof(protos))) {
                    return report_failure(L"Error setting secure protocols");
                }
            }
            ...
            if (!WinHttpSetTimeouts(m_hSession,
                ...
                milliseconds))
            {
                return report_failure(L"Error setting timeouts");
            }
            ...
            if (config.guarantee_order())
            {
                ...
                {
                    return report_failure(L"Error setting options");
                }
            }
            ...
            if (WINHTTP_INVALID_STATUS_CALLBACK == WinHttpSetStatusCallback(
                ...
                0))
            {
                return report_failure(L"Error registering callback");
            }
            ...
            if (m_hConnection == nullptr)
            {
                return report_failure(L"Error opening connection");
            }
        ...
        void send_request(_In_ const std::shared_ptr<request_context> &request)
        {
            ...
            const std::wstring encoded_resource = uri_builder(m_uri).append(msg.relative_uri()).to_uri().resource().to_string();
...
#pragma pop_macro ("max")
            ...
            {
                const std::wstring flattened_headers = flatten_http_headers(msg.headers());
    ...
    private:
        ...
        static void _multiple_segment_write_data(_In_ winhttp_request_context * p_request_context)
        {
            ...
            SafeInt<unsigned __int64> safeCount = p_request_context->m_remaining_to_write;
            ...
            if (rbuf.acquire(block, length))
            {
                if (length == 0)
                {
                    ...
                    {
                        p_request_context->report_error(GetLastError(), L"Error reading outgoing HTTP body from its stream.");
                    }
                    else
                    {
            ...
            else
            {
                    ...
                    [p_request_context, rbuf](pplx::task<size_t> op)
                {
                    ...
                    if (read == 0)
                    {
                        p_request_context->report_exception(http_exception(L"Unexpected end of request body stream encountered before Content-Length met."));
                        return;
                    }
            ...
            if (p_request_context != nullptr)
            {
                ...
                case WINHTTP_CALLBACK_STATUS_SENDREQUEST_COMPLETE:
                {
                    if (!p_request_context->m_request.body())
                    {
                        ...
                        if (progress)
                        {
                            try { (*progress)(message_direction::upload, 0); }
                            catch (...)
                            {
                                p_request_context->report_exception(std::current_exception());
                                return;
                ...
                case WINHTTP_CALLBACK_STATUS_WRITE_COMPLETE:
                {
                    ...
                    if (bytesWritten > 0)
                    {
                        ...
                        if (progress)
                        {
                            ...
                            try { (*progress)(message_direction::upload, p_request_context->m_uploaded); }
                            catch (...)
                            {
                                p_request_context->report_exception(std::current_exception());
                                return;
                ...
                case WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE:
                {
                    ...
                    wchar_t * header_buffer = reinterpret_cast<wchar_t *>(&header_raw_buffer[0]);
