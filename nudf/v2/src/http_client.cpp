...
void http::client::open()
{
    ...
    try {
        ...
        if (nullptr == _session_handle) {
            ...
            if (secure()) {
                const unsigned long protos = WINHTTP_FLAG_SECURE_PROTOCOL_TLS1
...
void http::client::send_request(const basic_request* req, basic_response* res)
{
    ...
    try {
        ...
        if (async()) {
            ...
            if (WAIT_OBJECT_0 != ::WaitForSingleObject(context->_complete_event, INFINITE)) {
                throw NX::exception(WIN32_ERROR_MSG2(GetLastError()), GetLastError());
            }
        }
        else {
...
void http::client::send_request_sync(basic_response* res, void* context)
{
    ...
    if (rc->_request->get_data_length() > 0) {
        ...
        do {
            ...
            const std::vector<unsigned char>& input = rc->_request->get_data(http::max_cache_size);
            if(input.empty()) {
                break;
            }
            if (!WinHttpWriteData(rc->_request_handle, input.data(), (unsigned long)input.size(), &bytes_written)) {
                throw NX::exception(WIN32_ERROR_MSG2(GetLastError()), GetLastError());
            }
        } while (true);
    }
...
http::string_request::string_request(const std::wstring& request_method,
    const std::wstring& request_path,
    const std::vector<std::pair<std::wstring, std::wstring>>& request_headers,
    const std::string& s) : basic_request(request_method, request_path, request_headers, std::vector<LPCWSTR>()), _body(s), _pos(0)
{
}
...
http::string_request::string_request(const std::wstring& request_method,
    const std::wstring& request_path,
    const std::vector<std::pair<std::wstring, std::wstring>>& request_headers,
    const std::wstring& s) : basic_request(request_method, request_path, request_headers, std::vector<LPCWSTR>()), _body(NX::conversion::utf16_to_utf8(s)), _pos(0)
{
}
...
std::vector<unsigned char> http::string_request::get_data(size_t length) const
{
    ...
    if (_pos < get_data_length()) {
        ...
        if (length > 0) {
            ...
            memcpy(read_buf.data(), _body.c_str() + _pos, length);
            _pos += length;
        }
    }
...
void http_detail::request_context::on_async_request_error(const WINHTTP_ASYNC_RESULT* result)
{
    ...
    case API_RECEIVE_RESPONSE:
        throw WIN32_ERROR_MSG((int)result->dwError, "WinHttpReceiveResponse");
    case API_QUERY_DATA_AVAILABLE:
        throw WIN32_ERROR_MSG((int)result->dwError, "WinHttpQueryDataAvaliable");
    case API_READ_DATA:
        throw WIN32_ERROR_MSG((int)result->dwError, "WinHttpReadData");
    case API_WRITE_DATA:
        throw WIN32_ERROR_MSG((int)result->dwError, "WinHttpWriteData");
    case API_SEND_REQUEST:
        throw WIN32_ERROR_MSG((int)result->dwError, "WinHttpSendRequest");
    default:
        throw WIN32_ERROR_MSG((int)result->dwError, "Unknown WinHTTP Function");
    }
}
...
void http_detail::request_context::on_async_send_complete()
{
    ...
    if (_request->get_data_length() > 0) {
        ...
        const std::vector<unsigned char>& input = _request->get_data(http::max_cache_size);
        if (input.empty()) {
            throw NX::exception(WIN32_ERROR_MSG2(ERROR_INVALID_DATA), ERROR_INVALID_DATA);
        }

        if (!WinHttpWriteData(_request_handle, input.data(), (unsigned long)input.size(), &bytes_written)) {
            throw NX::exception(WIN32_ERROR_MSG2(GetLastError()), GetLastError());
        }
    }
    else {
