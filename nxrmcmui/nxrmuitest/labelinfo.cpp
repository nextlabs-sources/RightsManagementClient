...
bool CLabel::Load(_In_ IXMLDOMNode* pNode, _In_opt_ CLabel* parent)
{
    ...
    try {
        ...
        for(long i=0; i<count; i++) {
            ...
            std::shared_ptr<CLabelValue> spLabelValue(new CLabelValue());
...
{
    for(std::vector<std::shared_ptr<CLabelValue>>::const_iterator it=_values.begin(); it!=_values.end(); ++it) {
...
bool CLabelValue::Load(_In_ IXMLDOMNode* pNode, _In_ CLabel* parent)
{
    ...
    try {
        ...
        std::shared_ptr<CLabel> spLabel(new CLabel());
...
CLabel* CLabelConf::LoadLabels(_In_opt_ const std::wstring& lang)
{
    HRESULT hr = S_OK;
    CComPtr<IXMLDOMNode> spLabels;
    CLabel* labels = NULL;
