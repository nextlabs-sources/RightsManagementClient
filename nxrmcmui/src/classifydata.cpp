...
CItem::CItem()
    : _selid(-1), _mandatory(false), _multisel(false)
{
}
...
void CClassifyData::LoadFromXml(const std::wstring& xml, const std::wstring& group_name) throw()
{
    nudf::util::CXmlDocument doc;
    doc.LoadFromXml(xml.c_str());
    Load(doc.GetDoc(), group_name);
}

void CClassifyData::Load(IXMLDOMDocument* doc, const std::wstring& group_name) throw()
{
    ...
    try {
        ...
        for(int i=0; i<nLabels; i++) {
            ...
            CItem item(id, name, displayname, mandatory, multisel);
...
std::wstring CClassifyData::expand_value(const std::wstring& value) const noexcept
{
    ...
    if (boost::algorithm::starts_with(value, L"$(") && boost::algorithm::ends_with(value, L")")) {
        ...
        auto pos = _dynamic_attributes.find(expanded_value);
    ...
    return std::move(expanded_value);
}
...
void CClassifyData::LoadFromJson(const std::wstring& ws, const std::wstring& groupname) throw()
{
    ...
    try {
        ...
        std::for_each(labels.begin(), labels.end(), [&](const std::pair<std::wstring,NX::web::json::value>& lb) {
            ...
            CItem item(label_id, label_name, label_display_name, label_mandatory, label_multi_select);
